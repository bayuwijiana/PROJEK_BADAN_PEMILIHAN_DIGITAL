...
@section('content')
    ...
    <body>
        ...
        <div class="card mt-3 oke ">
            <div class="card-content">
                @if ($status == 'sudah dihitung' && $role == 'Admin')
                    <div style="display: flex">
                        <a href="{{ route('suara.create') }}">
                            <button class="btn btn-warning">SELESAIKAN ACARA </button>
                        </a>
                    </div>
                @endif

                @if ($status == 'belum ada' && $role == 'Admin')
                    <a href="{{ route('pemilihan.create') }}">
                        <button class="btn btn-primary">Buat Kegiatan</button>
                    </a>
                @endif
                <h1>
                ...
                <h1>
                    @if ($status == 'belum dimulai' && $role == 'Admin')
                        <form action="/mulai" method="post">
                            @csrf
                            <button type="submit" class="btn btn-warning">Mulai Pencoblosan</button>
                        </form>
                    @endif
                </h1>
            ...
            <div class="card mt-3 oke">
                <div class="card-content">
                    ...
                    <table cellpadding="15px" style="font-size: 15px;font-weight:bold">
                        ...
                        <tr>
                            <td>
                                <a href="/cetak" target="blank" style="margin-left:10px">
                                    <button class="btn btn-info">CETAK
