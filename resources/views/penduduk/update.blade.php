...
@section('content')
    ...
    <body class="bg-theme bg-theme1">
        ...
        <form method="POST" action="{{ route('penduduk.update', $data->id) }}">
            @csrf
            @method('PUT')
            <div class="form-group">
                ...
                <div class="position-relative has-icon-right">
                    ...
                    <input id="nik" type="text" value="{{ $data->nik }}"
                        class="form-control @error('nik') is-invalid @enderror" name="nik" value="{{ old('nik') }}"
                        required autocomplete="nik" autofocus placeholder="MASUKAN NIK">
            ...
            <div class="form-group">
                ...
                <div class="position-relative has-icon-right">
                    @php
                        $jen=["LAKI LAKI","PEREMPUAN"];
                    @endphp
                    <select name="jenis_kelamin" class="form-control sel1">
                        @for ($x = 0; $x < 2; $x++)
                            <option value="{{ $jen[$x] }}" @if ($jen[$x] == $data->jenis_kelamin) selected @endif>
                                {{ $jen[$x] }}
                            </option>
                        @endfor
                    </select>
